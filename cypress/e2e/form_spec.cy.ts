describe("form page", () => {
  ...
  it("should be able to go to the saved questions page when clicked", () => {
    ...
    cy.get(".message").contains(
      "There are Currently no Saved Questions! Please Take a Quiz to Find Questions You Would like to Save"
    );
  });
});
...
describe("filling form out", () => {
  ...
  it("should be able to click start quiz and be take to quiz page with selected number of questions and topic", () => {
    ...
    cy.get('[tabindex="0"]').click();
    cy.get(".quiz").click();
    cy.get(":nth-child(1) > .card-button")
      .should("be.visible")
      .contains("Question 1");
    cy.get(":nth-child(2) > .card-button")
      .should("be.visible")
      .contains("Question 2");
    cy.get(":nth-child(3) > .card-button")
      .should("be.visible")
      .contains("Question 3");
    cy.get(":nth-child(4) > .card-button")
      .should("be.visible")
      .contains("Question 4");
    cy.get(":nth-child(5) > .card-button")
      .should("be.visible")
      .contains("Question 5");
  });
});
